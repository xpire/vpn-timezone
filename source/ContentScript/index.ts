...
const prefs = `
  ...
  try {
    ...
    // check if Date.prefs is populated, do not spoof if not enabled
    if (!Date.prefs) {
      console.log(Date.prefs, "Date.prefs is not set, so not spoofing")
      okay = false;
    }
  } catch (e) {
    console.log(Date.prefs, "Date.prefs errored")
    okay = false;
  }
`;
...
const shiftedDate = `
  const clean = str => {
    ...
    console.log("in clean:", {str})
    str = str.replace(/([T\\(])[\\+-]\\d+/g, '$1' + toGMT(Date.prefs[1]));
    console.log("in clean (2):", {str})
    if (str.indexOf(' (') !== -1) {
      str = str.split(' (')[0] + ' (' + Date.prefs[3] + ')';
    }
    console.log("in clean (3):", {str})
    return str;
  }
  const ODate = Date;
  const {
...
const enabledCheck = browser.storage.local.get("enabled");
enabledCheck.then(
  (success) => {
    console.log({ success });
    if (success.enabled) {
      ...
      script.remove();
      console.log("content script success", { enabledCheck, success });
    } else {
      console.log("content script success, not enabled", {
        enabledCheck,
        success,
      });
    }
